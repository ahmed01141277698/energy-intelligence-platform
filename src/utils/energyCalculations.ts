...
export class EnergyCalculations {
  // Physical constants
  static readonly GRID_CO2_FACTOR = 0.6; // kg CO2/kWh for Egyptian grid
  static readonly SOLAR_CO2_FACTOR = 0.04; // kg CO2/kWh lifecycle
  static readonly WIND_CO2_FACTOR = 0.01; // kg CO2/kWh lifecycle
  static readonly GENERATOR_CO2_FACTOR = 0.85; // kg CO2/kWh

  // Calculate electrical power using P = V * I * cos(φ) * √3 for 3-phase
  static calculateThreePhasePower(voltage: number, current: number, powerFactor: number): number {
    return Math.sqrt(3) * voltage * current * powerFactor / 1000; // kW
  }

  // Calculate energy consumption over time
  static calculateEnergyConsumption(power: number, hours: number): number {
    return power * hours; // kWh
  }

  // Calculate efficiency using η = (Useful Output / Total Input) * 100
  static calculateEfficiency(usefulOutput: number, totalInput: number): number {
    if (totalInput === 0) return 0;
    return (usefulOutput / totalInput) * 100;
  }

  // Calculate power losses using P_loss = I²R (simplified)
  static calculatePowerLosses(current: number, resistance: number): number {
    return Math.pow(current, 2) * resistance / 1000; // kW
  }

  // Calculate reactive power Q = V * I * sin(φ) * √3
  static calculateReactivePower(voltage: number, current: number, powerFactor: number): number {
  ...
  static calculateEnergyCost(consumption: number, tariffStructure: TariffStructure): number {
    let totalCost = 0;
  ...
  static predictConsumption(historicalData: number[], seasonalFactor: number = 1): PredictionResult {
  ...
  static calculateCO2Emissions(energySources: EnergySourceData[]): number {
  ...
  static getCO2Factor(sourceType: string): number {
    switch (sourceType) {
  ...
  static optimizeLoadDistribution(loads: LoadData[], constraints: OptimizationConstraints): OptimizationResult {
  ...
  static calculatePowerFactorCorrection(activePower: number, currentPF: number, targetPF: number): PowerFactorCorrection {
...
interface PowerFactorCorrection {
  capacitanceNeeded: number; // µF
  cost: number; // EGP
  savings: number; // EGP/month
}

// Egyptian electricity tariff structure (2024)
export const EGYPTIAN_TARIFF: TariffStructure = {
  tiers: [
