...
interface EnergySource {
  type: 'solar' | 'wind' | 'generator' | 'grid';
  capacity: number;
...
const appSlice = createSlice({
  ...
  reducers: {
    ...
    updateEnergySource: (state, action: PayloadAction<{ index: number; data: Partial<EnergySource> }>) => {
      const { index, data } = action.payload;
      if (state.energySources[index]) {
        state.energySources[index] = { ...state.energySources[index], ...data };
      }
    },
    updateFactoryZone: (state, action: PayloadAction<{ id: string; data: Partial<FactoryZone> }>) => {
    ...
    updateAIPredictions: (state, action: PayloadAction<Partial<AppState['aiPredictions']>>) => {
